...
const get = async (qs) => {
    ...
    assert_1.default.strictEqual(res.status, 200, `${qs} -> HTTP ${res.status}`);
    const body = await res.json();
    assert_1.default.ok(body.success, `${qs} -> success=false`);
    return body.data;
};
(async () => {
    for (const flag of ['', 'gifting=true', 'newArrival=true']) {
        ...
        assert_1.default.ok(Array.isArray(facets.categories), `${label}: categories must be an array`);
        assert_1.default.ok(Array.isArray(facets.brands), `${label}: brands must be an array`);
        for (const name of facets.categories) {
            const hits = await get(`${flag}&category=${encodeURIComponent(slug(name))}`);
            assert_1.default.ok(hits.length > 0, `${label}: category "${name}" is offered but returns nothing`);
        }
        for (const name of facets.brands) {
            const hits = await get(`${flag}&brand=${encodeURIComponent(slug(name))}`);
            assert_1.default.ok(hits.length > 0, `${label}: brand "${name}" is offered but returns nothing`);
        }
...
//# sourceMappingURL=collectionFilters.test.js.map